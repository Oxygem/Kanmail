...
def make_fake_addresses():
    ...
    return tuple([make_fake_address() for _ in range(n_addresses)])
...
def get_fake_fetch_item(folder, uid, keys, imap_host):
    ...
    return {key: value for key, value in fake_data.items() if key in keys}
...
class FakeFolderData(object):
    ...
    def remove_uids(self, uids):
        self.uids = [uid for uid in self.uids if uid not in uids]
