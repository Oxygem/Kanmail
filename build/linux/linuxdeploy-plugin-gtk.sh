...
# sources:
# - original: https://raw.githubusercontent.com/linuxdeploy/linuxdeploy-plugin-gtk/master/linuxdeploy-plugin-gtk.sh
# - tauri fork: https://raw.githubusercontent.com/tauri-apps/linuxdeploy-plugin-gtk/master/linuxdeploy-plugin-gtk.sh
# - wails fork: https://github.com/wailsapp/wails/blob/v3-alpha/v3/internal/commands/linuxdeploy-plugin-gtk.sh
# based on tauri version with modifications, none of the above worked without changes!
...
# Enable for debugging (gross!)
#echo "DIR IS: $(pwd)"
#echo "ENV IS: $(env)"
#echo "APPDIR IS: $APPDIR"

# Both of these are required for libwebkit to find the subprocesses it uses
cd $APPDIR
...
for directory in "${PATCH_ARRAY[@]}"; do
    while IFS= read -r -d '' file; do
        echo ln $verbose -s "${file/\/usr\/lib\//}" "$APPDIR/usr/lib"
        ln $verbose -s "${file/\/usr\/lib\//}" "$APPDIR/usr/lib" || echo failed
